...
class MentorfeedbackForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'subjects' : forms.Textarea(attrs={'style':'resize:none;', 'rows':4, 'placeholder':'과학을 좋아하지만 성적이 잘 나오지는 않고 수학은 잘 합니다. 국어 성적과 사회 성적은 잘 안나오는 편. 학생은 생각하고 논리적으로 추론하는 것을 좋아하지만 암기하는 것은 잘 못합니다. 책을 많이 읽지 않아 글 읽는 속도가 느리고 문맥을 빨리 파악하지 못해 국어와 사회 점수가 잘 안나오고 과학 또한 흥미에 비해서는 점수가 잘 나오지 않습니다. 독서 습관이 없어 좋아하는 책에 대한 독서를 시작으로 하여 책읽는 습관을 붙이고 이를 바탕으로 관련 교과랑 연결지어 폭넓은 공부를 하는 것이 장기적으로 도움이 될 것입니다 등'}),
            'future_path' : forms.Textarea(attrs={'style':'resize:none;', 'rows':4, 'placeholder':'어릴적 우연히 보게된 영화 ‘어벤저스’를 보고 영웅들에 대해 관심 가지게 되었고, 과학을 통해 인간의 능력을 무한히 끌어올리는 것에 대해 신기함과 동시에 흥미를 느끼게 되었습니다. 영화속 등장인물이 정말 가능할지 관련 문헌을 인터넷 웹페이지에서 시작해서, 뉴스기사, 과학 저널, 논문으로 이어진다면 자연스럽게 독서, 논문으로 이어질 것이고 과학에 대한 탄탄한 지식과 읽고 쓰는 국어능력 아울러 사회적 측면까지 고려하게 되는 눈을 가지게 될 것입니다.  영화만 계속 본다면 문제가 있겠지만 이런 종류의 영화가 자극이 되어 좀 더 학문의 세계와 학교공부로 이어질 수 있다면 원하는 꿈에 한 발자국 더 다가갈 수 있을 것입니다. 학교에서 국어, 과학, 사회 시간에 이런 주제로 조별활동, 개별 발표가 종종 있습니다. 그때 학생들에게 인정받으며 자신감을 키우고 생활기록부에 진로, 특기사항으로 기록되어 차후 자소서나 추천서 쓸 때 도움이 될 것입니다 등'}),
            'lifestyle' : forms.Textarea(attrs={'style':'resize:none;', 'rows':4, 'placeholder':'학교- 학원-집 의 규칙적인 생활을 하며 성실하게 임하고 있습니다. 하지만 학원에 밤 12시까지 있으면서 ~~~입니다. 오랜 시간 앉아서 공부하는 것보다 하루하루의 목표량을 정하고 달성하면 쉴수 있는 보상을 주어 좀 더 효율적인 공부를 할 수 잇을 것입니다 등'}),
            'sw' : forms.Textarea(attrs={'style':'resize:none;', 'rows':4, 'placeholder':'자신이 흥미를 가지고 꽃히면 물불 가리지 않고 빠져들어가는 성향이 있습니다. 하지만 관심이 없는 것에 대해서는 집중을 할 수 없고 딴생각을 합니다. 관심 없는 것에 흥미를 느낄 수 있도록 하는 것이 가장 좋은 방법이지만 현실적으로는 단기적인 목표를 설정하여 필요한 수준까지는 해낼 수 있는 동기부여가 필요합니다 등'}),
            'reference' : forms.Textarea(attrs={'style':'resize:none;', 'rows':4, 'placeholder':'너무 공부공부 하면서 실증 느끼게 하는 것보다 적당히 휴식과 동기를 주면서 스스로 흥미를 느껴 할 수 있는 환경이 만들어지면 좋을 것 같습니다 등'}),
        }
